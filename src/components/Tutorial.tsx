...
const addKingsToFen = (fen: string) => {
    ...
    let modifiedBoardStr = boardStr;

    if (!whiteKingPlaced) {
        modifiedBoardStr = modifiedBoardStr.replace(/[1-8]/, (match) => {
    ...
    return fen.replace(boardStr, modifiedBoardStr);
};
...
const Tutorial = ({ pieceTheme }: { pieceTheme: 'zoo' | 'standard' }) => {
    ...
    const handleMove = (move: { from: string; to: string; promotion?: string }) => {
        try {
            ...
            if (result) {
                const currentFen = game.fen().replace(/ (b|w) /, ' w ');
