...
const Tutorial = ({ pieceTheme }: { pieceTheme: 'zoo' | 'standard' }) => {
    ...
    const handleMove = (move: { from: string; to: string; promotion?: string }) => {
        try {
            ...
            if (result) {
                // HACK: We use a shallow clone and directly mutate the internal `_turn` property
                // because `chess.js` strictly validates FEN strings and rejects them if they are
                // missing kings. Faking kings and parsing a FEN string is computationally expensive.
                // This brittle shallow copy avoids those overheads but relies on `chess.js` internals.
