...
import { addKingsToFen, removeKings, initGame } from '../utils/tutorialHelpers';
...
const Tutorial = ({ pieceTheme }: { pieceTheme: 'zoo' | 'standard' }) => {
    ...
    const [game, setGame] = useState(() => initGame(tutorials[0].fen, tutorials[0].id));
    /* eslint-disable-next-line @typescript-eslint/no-unused-vars */
    const [_, setFen] = useState(game.fen());

    const handleSelectTutorial = (t: typeof tutorials[0]) => {
        setActiveTutorial(t);
        const newGame = initGame(t.fen, t.id);
        setGame(newGame);
        setFen(newGame.fen());
    };
        ...
        <div className="game-layout">
            ...
            <aside className="info-panel">
                ...
                <button className="btn-secondary" onClick={() => {
                    const resetGame = initGame(activeTutorial.fen, activeTutorial.id);
                    setGame(resetGame);
