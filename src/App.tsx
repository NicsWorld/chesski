...
import { getInitialAppState } from './utils/initAppState';
import './App.css';

function App() {
  const { initialView, initialGame, initialFen } = getInitialAppState();
  const [view, setView] = useState<'game' | 'tutorial'>(initialView);
  const [game, setGame] = useState(initialGame);
  const [pieceTheme, setPieceTheme] = useState<'zoo' | 'standard'>('zoo');
  /* eslint-disable-next-line @typescript-eslint/no-unused-vars */
  const [_, setFen] = useState(initialFen);
